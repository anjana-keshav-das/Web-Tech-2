...
def add_user():
	...
	if q != None:
		...
		if is_sha1(request.json['password']):
			output = {'username': request.json['username'],
...
def get_user(username):
    ...
    if(val == None):
        ...
        if(q==None):
            ...
            print("User found")
            return jsonify({'username':q['username'], 'password':q['password']}),200
    else:
        details = db.details
        ...
        if(q==None):
            ...
            output = {'username':q['username'],'name':q['name'],'contact':q['contact'],'address':q['address'],'email':q['email']}
...
def add_user_details(username):
    ...
    if(val == None):
        ...
        if(q==None): #add details
            res = {'username':username,'name':request.json['name'],'contact':request.json['contact'],'address':request.json['address'],'email':request.json['email']}
            q = details.insert(res)
            print(res, ": details inserted")
            return jsonify({}), 201
        else:
            q = details.update({'username':username},{'$set':{request.json['field']:request.json['value']}})
            if(q['n'] == 0):
                print("Update failed")
                return jsonify({}),400
        ...
        if is_sha1(request.json['password']):
            q = login.update({'username':username},{'$set':{'password':request.json['password']}})
            if(q['n'] == 0):
                print("Update failed")
                return jsonify({}),400
            print("Update successful")
